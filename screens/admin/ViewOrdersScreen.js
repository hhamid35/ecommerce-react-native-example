...
const ViewOrdersScreen = ({ navigation, route }) => {
  ...
  return (
    <View style={styles.container} testID="view-orders-screen">
      <ProgressDialog visible={isloading} label={label} />
      <StatusBar testID="view-orders-status-bar"></StatusBar>
      <View style={styles.TopBarContainer}>
        <TouchableOpacity
          testID="view-orders-back-btn"
          onPress={() => {
            navigation.goBack();
          }}
        >
      ...
      <View style={styles.screenNameContainer}>
        <View>
          <Text style={styles.screenNameText} testID="view-orders-heading">View Order</Text>
        </View>
        <View>
          <Text style={styles.screenNameParagraph} testID="view-orders-subtitle">View all orders</Text>
        </View>
      </View>
      <CustomAlert message={error} type={alertType} testID="view-orders-alert" />
      <CustomInput
        radius={5}
        placeholder={"Search..."}
        value={filterItem}
        setValue={setFilterItem}
        testID="view-orders-search-input"
      />
      <ScrollView
        testID="view-orders-scroll"
      ...
      >
        {foundItems && foundItems.length == 0 ? (
          <Text testID="view-orders-empty-text">{`No order found with the order # ${filterItem}!`}</Text>
        ) : (
